...
export default function SettingsModal({ isOpen, onClose }: SettingsModalProps) {
  ...
  const updateMutation = trpc.auth.updateCredentials.useMutation({
    onSuccess: () => {
      setSuccess(language === "rs" ? "Ažurirano. Prijavi se ponovo." : "Updated. Please log in again.");
  ...
  const handleSubmit = () => {
    ...
    if (!currentPassword) {
      setError(language === "rs" ? "Unesi trenutnu lozinku" : "Current password is required");
      return;
    }

    if (newPassword && newPassword !== confirmPassword) {
      setError(language === "rs" ? "Nove lozinke se ne poklapaju" : "New passwords do not match");
      return;
    }

    if (newPassword && newPassword.length < 6) {
      setError(language === "rs" ? "Lozinka mora imati bar 6 znakova" : "Password must be at least 6 characters");
      return;
    }
