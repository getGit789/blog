...
/**
 * Smooth bottom falloff for the shader.
 *
 * The animation used to end on a hard horizontal edge wherever the column
 * stopped. Instead it now holds at full strength down to `fadeStart` and eases
 * out to nothing at the bottom. The stops trace a smoothstep curve rather than
 * a straight line, because a plain linear ramp reads as a visible seam right
 * where it begins.
 */
function fadeMask(fadeStart: number): string {
  const steps = 10;
  const stops: string[] = [`rgba(0,0,0,1) 0%`, `rgba(0,0,0,1) ${(fadeStart * 100).toFixed(1)}%`];
  for (let i = 1; i <= steps; i++) {
    const t = i / steps;
    const smooth = t * t * (3 - 2 * t); // smoothstep
    const alpha = 1 - smooth;
    const pos = fadeStart + (1 - fadeStart) * t;
    stops.push(`rgba(0,0,0,${alpha.toFixed(3)}) ${(pos * 100).toFixed(1)}%`);
  }
...
interface ShaderCanvasProps {
  /**
   * Fraction of the height at which the fade begins. 0.75 keeps the animation
   * solid through the top three quarters of the hero and fades across the last
   * quarter.
   */
  fadeStart?: number;
}

export default function ShaderCanvas({ fadeStart = 0.75 }: ShaderCanvasProps) {
  ...
  useEffect(() => {
    ...
    // hero is sized by its content. Sizing the canvas only once left the
    // animation covering part of the column and cutting off mid page, so track
    // the element itself rather than just the window.
